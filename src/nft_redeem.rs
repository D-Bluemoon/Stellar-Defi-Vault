...
//! receipt, which is exactly the receipt tied to their address — there is no
...
//! which already exist but were never wired to a public entrypoint — added
...
impl VaultContract {
    ...
    ///
    /// Required before `burn_and_redeem` can be used — mirrors how
    ...
    ///
    /// Pending reward is settled first — exactly like the regular unstake
    /// flow — then the full principal is returned, and finally the receipt
    ...
    pub fn burn_and_redeem(env: Env, user: Address) -> Result<i128, VaultError> {
        ...
        if lock_period > 0 {
            ...
            if env.ledger().sequence() < staked_at.saturating_add(lock_period) {
                return Err(VaultError::PositionStillLocked);
            }
        }
